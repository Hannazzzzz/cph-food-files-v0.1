...
const BakeryMap = ({
  ...
  useEffect(() => {
    ...
    // Zoom to level 16 where clustering is disabled
    const targetZoom = 16;

    // Open popup after zoom completes
    const openPopup = () => {
      // Wait a bit for cluster group to update after zoom
      setTimeout(() => {
      ...
      */}
      <style>{`
        ...
        */
        ...
        [data-map-theme="cph-food-files"] .cluster-popup::-webkit-scrollbar {
          width: 8px;
        }

        [data-map-theme="cph-food-files"] .cluster-popup::-webkit-scrollbar-track {
        ...
        <TileLayer
          ...
          url={
            import.meta.env.VITE_CARTO_API_KEY
              ? `https://{s}.basemaps.cartocdn.com/rastertiles/light_all/{z}/{x}/{y}{r}.png?key=${import.meta.env.VITE_CARTO_API_KEY}`
